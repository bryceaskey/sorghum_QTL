%stalk_segmentation_v2 - testing with Dijkstra's shortest path algorithim
...
stalk_bot = [pot_top, round(min(point_means))];
...
%erode with vertical line element
% SE = strel('line', 10, 90);
% all_plant_eroded = imerode(all_plant, SE);
% all_plant_eroded = bwareaopen(all_plant_eroded, 300);
...
%median_plant = rmoutliers(median_plant);

radius = 1;
pixel = stalk_bot;
while median_plant(pixel(1), pixel(2)) ~= 1

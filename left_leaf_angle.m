...
function [left_angle_data, left_rearranged, left_coefficients] = left_leaf_angle(segmented_image, left_leaf_count, test_lines, shape_coeffs, width_coeffs)
...
left_leaf_data = {}; %each row contains intersect data for 1 line
for dist = 1:1:test_lines %test test_lines out from left edge of stem
    ...
    for row = leaf_top:1:leaf_bot
        ...
        elseif leaves(pixel(1), pixel(2)) == 0 && leaf_bin == 1 %end of current leaf - save array to cell
            left_leaf_data(dist, count) = {leaf_data};
            leaf_bin = 0;
        else
            leaf_bin = 0;
        end
...
[line_count, leaf_count] = size(left_leaf_data);

%create matrix containing mean position values from left_leaf_data
...
for row = 1:1:line_count
    for col = 1:1:leaf_count
        if isempty(left_leaf_data{row, col}) == 0
            points = left_leaf_data{row, col};
    ...
    for row = start - 1:-1:1
        ...
        for ii = 1:1:next_num_points
            for jj = 1:1:prev_num_points
                row_count = 0;
                if isnan(left_rearranged(row + 1, jj, 1)) && isnan(left_rearranged(row + 1, jj, 2)) %cycle down rows in a col until a cell ~= NaN is found
                    while isnan(left_rearranged(row + 1 + row_count, jj, 1)) && isnan(left_rearranged(row + 1 + row_count, jj, 2))
                        row_count = row_count + 1;
                    end
                end
                x1 = left_rearranged(row + 1 + row_count, jj, 1);
                y1 = left_rearranged(row + 1 + row_count, jj, 2);
        ...
        for points = 1:1:next_num_points
            ...
            left_rearranged(row, current_ind, 1) = mean_coordinates(row, previous_ind, 1);
            left_rearranged(row, current_ind, 2) = mean_coordinates(row, previous_ind, 2);
            geo_dist(previous_ind, :) = NaN;
            geo_dist(:, current_ind) = NaN;
            
        end
        for col = 1:1:leaf_count
            if left_rearranged(row, col, 1) == 0 && left_rearranged(row, col, 2) == 0 %missing point - fill with NaN
                left_rearranged(row, col ,1) = NaN;
                left_rearranged(row, col, 2) = NaN;
            end
        end
    end
end
...
if start ~= line_count %starting row is not bottom row - need to iterate down matrix
     for row = start + 1:1:line_count
        ...
        for ii = 1:1:next_num_points
            for jj = 1:1:prev_num_points
                row_count = 0;
                if isnan(left_rearranged(row - 1, jj, 1)) && isnan(left_rearranged(row - 1, jj, 2)) %cycle up rows in a col until a cell ~= NaN is found
...
for col = 1:1:leaf_count
    [~, row_outliers] = rmoutliers(left_rearranged(:, col, 1));
    [~, col_outliers] = rmoutliers(left_rearranged(:, col, 2));
    for row = 1:1:line_count
        if row_outliers(row) == 1 || col_outliers(col) == 1
            left_rearranged(row, col, :) = NaN;
        end
    end
end
...
left_coefficients = zeros(leaf_count, 4); %each row contains data for a line
...
for col = 1:1:leaf_count %need to create new array without NaN values
    point_count = sum(~isnan(left_rearranged(:, col, 1)));
    line_points = [];
    valid_point_count = 0;
    for row = 1:1:line_count %cycle through each row of rearranged, skipping NaN cells
        if ~isnan(left_rearranged(row, col, 1))
            valid_point_count = valid_point_count + 1;
            line_points(valid_point_count, 1) = left_rearranged(row, col, 1);
            line_points(valid_point_count, 2) = left_rearranged(row, col, 2);
        end
    end
    if ~isempty(line_points)
        r_squared = (corr(line_points(:, 1), line_points(:, 2))).^2;
        left_coefficients(col, 1) = valid_point_count;
        left_coefficients(col, 2) = r_squared;
        line = polyfit(line_points(:, 1), line_points(:, 2), 1);
        left_coefficients(col, 3) = line(1);
        left_coefficients(col, 4) = line(2);
    end
end

%ignore lines that don't contain >= 1/3 of total lines tested
valid_lines = [];
valid_line_count = 0;
for row = leaf_count:-1:1
    if left_coefficients(row, 1) >= test_lines/4
        valid_line_count = valid_line_count + 1;
        valid_lines(valid_line_count, :) = left_coefficients(row, :);
    end
end
...
left_angle_data = []; %contains intersection coordinates, and angle between leaf and stalk lines
for line_count = 1:1:left_leaf_count %select lines with greatest r^2 value until number of lines matches left_leaf_count
    x = 1:1:height;
    ...
    left_angle_data = [left_angle_data; intersection(1), intersection(2), angle];
end

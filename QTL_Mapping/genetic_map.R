...
all_sorghum_data <- read.cross(format = "csv", file = "with_pheno.csv", na.strings="-", genotypes = c("AA", "BB"), alleles = c("A", "B"), crosstype = "riself")
...
map <- pull.map(as.map)
sorghum.csv <- read.csv(file="/home/schnable/Documents/R/QTL_Practice/with_pheno.csv", header=TRUE, sep=",", stringsAsFactors = FALSE)
new.sorghum.csv <- data.frame(sorghum.csv[ , 1], sorghum.csv[ , 2], sorghum.csv[ , 3])
names(new.sorghum.csv) <- names(sorghum.csv[ , 1:3])
for(i in 1:nchr(map)){
